...
const TaskOperatore=()=>{
 ...
 return <section className="main-section">
  ...
  <div className="box-checkin-checkout"><div><div className="box-timer"><div className="timer-value">{oraCorrente}</div><div className="timer-label">ORARIO CORRENTE</div></div><Button onClick={()=>{}} className="check-button check-button-in" size="lg"><span className="check-button-content"><MapPinCheckInside className="!w-[20px] !h-[20px]"/>CHECK-IN</span></Button></div></div>
  {turniGiornalieri.map((turno,index)=>{const stato=getStatoTurno(turno.dataTurno,turno.oraInizio,turno.oraFine);return <Card className={`card-turni ${index%2===1?'card-turni-alt':''}`} key={turno.idTurno}><CardContent className="turno-content"><div className="info-orario-turno"><div className="info-ora"><div className={`stato-turno ${stato}`}>{stato==='IN_CORSO'?'TURNO IN CORSO':stato==='FUTURO'?'PROSSIMO TURNO':'TURNO PASSATO'}</div><div className="orario-turno">{turno.oraInizio} - {turno.oraFine}</div></div><div className="info-pausa"><div className="pausa-label"><Coffee className="h-4 w-4 text-[#ccffec]" strokeWidth={1.5}/><span>Pausa</span></div><div className="pausa-value">{turno.orePausa} ora</div></div></div><div className="info-turno"><div className="titolo-evento">{turno.titoloEvento}</div><div className="luogo-evento-content"><MapPin className="h-6 w-6" style={{color:'#a5e8cf'}}/><span className="luogo-evento-text">{turno.localitaEvento}</span></div><div className="tipo-evento">Tipo evento: {turno.tipologiaTurno}</div><div className="tipo-mansione">Mansione: {turno.tipoMansione}</div>{turno.notaTurno&&<div className="note-operative-content"><div className="note-operative-label">NOTE OPERATIVE</div><div className="note-operative-value">{turno.notaTurno}</div></div>}</div><div className="info-personale-content"><div className="colleghi-evento" onClick={()=>apriListaColleghi(turno.idTurno)}><Users className="h-7 w-7 text-[#00ffb8]" strokeWidth={1.5}/><span className="colleghi-evento-label">Colleghi</span></div><div className="referente-evento" onClick={()=>apriReferente(turno.idTurno)}><UserStar className="h-7 w-7 text-[#00ffb8]" strokeWidth={1.5}/><span className="referente-evento-label">Referente evento</span></div></div></CardContent></Card>})}
  <div className="sintesi-container"><div className="ore-mese-sintesi"><div className="ore-mese-label">ORE DEL MESE</div><div className="ore-mese-value-content"><CalendarClock className="h-7 w-7 text-[#00d96f]" strokeWidth={1.5}/><div className="ore-mese-value">123:30</div><div className="ore-label"> ore</div></div><div className="dettaglio-mese-sintesi" onClick={()=>navigate('/operator/rendicontazione')}>Vai al dettaglio</div></div><div className="prossimi-turni-sintesi"><div className="prossimi-turni-label">PROSSIMI TURNI</div><div className="prossimi-turni-row"><div className="prossimi-turni-next-content"><div className="prossimi-turni-mese-value">SET</div><div className="prossimi-turni-giorno-value">08</div></div><div className="prossimi-turni-value-content"><div>3 turni</div><div>previsti</div></div></div><div className="dettaglio-turni-sintesi" onClick={()=>navigate('/operator/turniFuturi')}>Vedi tutti</div></div></div>
